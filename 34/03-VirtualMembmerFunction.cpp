...
class Quadrilateral
{
    ...
    public: 
        ...
        {
            std::cout << "----Entered Quadrilateral::area()----" << std::endl; 
            double s = (s1 + s2 + s3 + s4)/2; 
            std::cout << "----Leaving Quadrilateral::area()----" << std::endl; 
...
void test(void)
{
    ...
    double P = pQ->perimeter();    // Quadrilateral::perimeter(pQ) 
    double A = pQ->area();         // Quadrilateral::area(pQ); 
    reinterpret_cast<Square*>(pQ)->diagonalLength(); // CTE 
